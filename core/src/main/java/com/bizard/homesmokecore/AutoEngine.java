...
public final class AutoEngine {
    ...
    public synchronized Update stop(String why) {
        ...
        state=State.ABORTED; reason=why==null?"Остановлено пользователем":why;
        return new Update(list("a3","x0"),reason,true);
    }

    public synchronized Update onTelemetry(Telemetry t,long nowMs) {
        if(state!=State.RUNNING) return Update.none(reason);
        AutoStage s=current();
        ...
        // Hold time means actual time held inside the allowed chamber band.
        if(chamberReady && inBand && s.usesTime()) accumulatedHoldMs+=dt;
        ...
        if(probeAllowed) {
            if(s.usesK()) probeDone=t.probeK>=s.probeTarget;
            else if(s.usesT()) probeDone=t.probeT>=s.probeTarget;
        }
        boolean complete;
        switch(s.finishCondition) {
            case TIME: complete=timeDone; break;
            case PROBE_K:
            case PROBE_T: complete=probeDone; break;
            case TIME_OR_K:
            case TIME_OR_T: complete=timeDone||probeDone; break;
            case TIME_AND_K:
            case TIME_AND_T: complete=timeDone&&probeDone; break;
        ...
        int next=nextEnabled(stageIndex);
        if(next<0) return finish("Программа завершена");
    ...
    public synchronized String statusText(double chamber,double k,double t) {
        if(state!=State.RUNNING) return reason.length()==0?state.name():reason;
        AutoStage s=current(); StringBuilder b=new StringBuilder();
        ...
        if(s.usesT()&&!Double.isNaN(t)) b.append(" · T ").append(one(t)).append("/").append(one(s.probeTarget)).append("°C");
        return b.toString();
    }
    ...
    private static String clock(long ms){long s=Math.max(0,ms/1000);long h=s/3600,m=(s%3600)/60,q=s%60;return h>0?String.format(java.util.Locale.US,"%02d:%02d:%02d",h,m,q):String.format(java.util.Locale.US,"%02d:%02d",m,q);}

    private static void validate(AutoProgram p) {
        if(p==null) throw new IllegalArgumentException("Программа не задана");
        if(p.stages.size()==0||p.stages.size()>4) throw new IllegalArgumentException("Допустимо 1..4 этапа");
        boolean any=false;
        for(int i=0;i<p.stages.size();i++) {
            AutoStage s=p.stages.get(i); if(!s.enabled) continue; any=true;
            if(s.chamberTarget<0||s.chamberTarget>100) throw new IllegalArgumentException("Этап "+(i+1)+": температура 0..100");
            if(s.tolerance<0||s.tolerance>10) throw new IllegalArgumentException("Этап "+(i+1)+": допуск 0..10");
            if(s.stableSeconds<0||s.stableSeconds>3600) throw new IllegalArgumentException("Этап "+(i+1)+": стабилизация 0..3600 сек");
            if(s.holdMs<0||s.holdMs>24L*60L*60L*1000L) throw new IllegalArgumentException("Этап "+(i+1)+": выдержка до 24 ч");
            if((s.usesK()||s.usesT())&&(s.probeTarget<0||s.probeTarget>100)) throw new IllegalArgumentException("Этап "+(i+1)+": щуп 0..100");
        }
        if(!any) throw new IllegalArgumentException("Нет активных этапов");
    }

    private static AutoProgram cloneProgram(AutoProgram src) {
        AutoProgram p=new AutoProgram(); p.stages.clear(); p.id=src.id;p.name=src.name;p.description=src.description;p.modifiedAt=src.modifiedAt;
        for(AutoStage s:src.stages)p.stages.add(s.copy()); return p;
    }
}
