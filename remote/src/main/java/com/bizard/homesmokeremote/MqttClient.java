...
final class MqttClient {
    ...
    synchronized void connect() throws IOException {
        ...
        writeUtf(b,"HomeSmokeRemote_"+Long.toHexString(System.nanoTime()));
    ...
    synchronized void publish(String topic,String payload,boolean retain)throws IOException{
        ...
        b.write(payload.getBytes(StandardCharsets.UTF_8));
        sendPacket(0x32|(retain?1:0),b.toByteArray());
    }
    ...
    synchronized void subscribe(String topic)throws IOException{
        ...
        ByteArrayOutputStream b=new ByteArrayOutputStream();
        int id=nextPacketId(); b.write((id>>>8)&255); b.write(id&255);
        writeUtf(b,topic.trim()); b.write(1);
    ...
    private void startKeepAlive(){
        keepAlive=new Thread(()->{
            while(connected){
                try{
                    Thread.sleep(20000);
                    synchronized(MqttClient.this){if(connected)sendPacket(0xC0,new byte[0]);}
    ...
    private static int readRemaining(InputStream in)throws IOException{
        int m=1,v=0,d;
        do{d=in.read();if(d<0)throw new EOFException();v+=(d&127)*m;m*=128;if(m>128*128*128*128)throw new IOException("bad remaining length");}while((d&128)!=0);
        return v;
    }
