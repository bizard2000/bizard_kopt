...
/** Local sessions, operational journal and per-session export. */
public final class HistoryActivity extends Activity {
    ...
    private void buildSessions(LinearLayout page){
        LinearLayout intro=card();intro.addView(text("Сеансы копчения",18,true,TEXT));intro.addView(detail("Сеанс начинается автоматически по свежей телеметрии. Короткие потери связи не разрывают сеанс."));page.addView(intro,margin(8,4,8,4));
    ...
    private void buildEvents(LinearLayout page){
        LinearLayout c=card();c.addView(text("Журнал событий Remote",18,true,TEXT));
        List<OperationalHistoryStore.Event> events=ops.queryEvents(100);
        if(events.isEmpty())c.addView(detail("Событий пока нет"));
        else{
            SimpleDateFormat f=new SimpleDateFormat("dd.MM HH:mm:ss",Locale.getDefault());
            for(OperationalHistoryStore.Event e:events){TextView row=text(f.format(new Date(e.ts))+" · "+e.message,12,false,eventColor(e.type));row.setPadding(0,dp(5),0,dp(5));c.addView(row);}
        }
    ...
    @Override protected void onActivityResult(int requestCode,int resultCode,Intent data){
        ...
        if(resultCode!=RESULT_OK||data==null||data.getData()==null||exportSession==null)return;
        try{writeExport(data.getData(),exportSession,requestCode==REQ_CSV?"csv":"json");Toast.makeText(this,"Экспорт сохранён",Toast.LENGTH_SHORT).show();ops.addEvent(System.currentTimeMillis(),"info","Экспортирован сеанс · "+(requestCode==REQ_CSV?"CSV":"JSON"));}
    ...
    private void writeExport(Uri uri,OperationalHistoryStore.Session s,String format)throws Exception{
        long end=s.active()?System.currentTimeMillis():s.endTs;List<TelemetryHistoryStore.Sample> samples=telemetry.query(s.startTs,end,0);
        ...
        try(OutputStreamWriter w=new OutputStreamWriter(os,"UTF-8")){
            if("csv".equals(format))writeCsv(w,s,samples);else writeJson(w,s,samples);
        }
    }
    ...
    private static String range(double min,double max,String suffix){if(Double.isNaN(min)||Double.isNaN(max))return "—";return one(min)+"…"+one(max)+suffix;}
    private static String value(double v,String suffix){return Double.isNaN(v)||Double.isInfinite(v)?"—":one(v)+suffix;}
    private static String one(double v){return String.format(Locale.getDefault(),"%.1f",v);}
    ...
    private LinearLayout card(){LinearLayout c=new LinearLayout(this);c.setOrientation(LinearLayout.VERTICAL);c.setPadding(dp(12),dp(10),dp(12),dp(10));c.setBackground(roundStroke(CARD,18,BORDER,1));if(android.os.Build.VERSION.SDK_INT>=21)c.setElevation(dp(1));return c;}
