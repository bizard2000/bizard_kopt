...
public final class SessionDetailActivity extends Activity {
    ...
    private View buildSummary(){
        ...
        c.addView(detail("Макс. ТЭН · "+value(session.heaterMax," %")+(session.outageMs>0?" · потери связи ≈ "+duration(session.outageMs):" · потерь связи не зафиксировано")));
    ...
    private View buildGraph(){
        ...
        TemperatureChartView chart=new TemperatureChartView(this);chart.setSeries(true,true,true,true);chart.setData(samples);
        c.addView(chart,new LinearLayout.LayoutParams(-1,dp(310)));
        pointText=detail("Коснитесь графика, чтобы увидеть значения точки.");pointText.setPadding(0,dp(5),0,0);c.addView(pointText);
        chart.setOnSelectionListener(s->pointText.setText(dateTime(s.ts)+"\nКамера "+num(s.camera)+" °C · Уставка "+num(s.setpoint)+" °C\nK "+num(s.probeK)+" °C · T "+num(s.probeT)+" °C · ТЭН "+num(s.heater)+" %"));
        return c;
    }

    private View buildEvents(){
        LinearLayout c=card();c.addView(text("События этого сеанса",18,true,TEXT));
        long end=session.active()?System.currentTimeMillis():session.effectiveEnd()+5000L;
        List<OperationalHistoryStore.Event> events=ops.queryEvents(Math.max(0,session.startTs-2000L),end,200);
        ...
        for(OperationalHistoryStore.Event e:events){
            TextView row=text(f.format(new Date(e.ts))+" · "+e.message,12,false,eventColor(e.type));row.setPadding(0,dp(5),0,dp(5));c.addView(row);
        }
        return c;
    }

    private void requestExport(String format){
        Intent i=new Intent(Intent.ACTION_CREATE_DOCUMENT);i.addCategory(Intent.CATEGORY_OPENABLE);i.setType("csv".equals(format)?"text/csv":"application/json");
        String stamp=new SimpleDateFormat("yyyyMMdd_HHmm",Locale.US).format(new Date(session.startTs));i.putExtra(Intent.EXTRA_TITLE,"HomeSmoke_Remote_session_"+stamp+"."+format);
        startActivityForResult(i,"csv".equals(format)?REQ_CSV:REQ_JSON);
    }

    @Override protected void onActivityResult(int requestCode,int resultCode,Intent data){
        super.onActivityResult(requestCode,resultCode,data);
        if(resultCode!=RESULT_OK||data==null||data.getData()==null)return;
        String format=requestCode==REQ_CSV?"csv":"json";
        try{writeExport(data.getData(),format);Toast.makeText(this,"Экспорт сохранён",Toast.LENGTH_SHORT).show();ops.addEvent(System.currentTimeMillis(),"info","Экспортирован сеанс · "+format.toUpperCase(Locale.ROOT));}
    ...
    private void writeExport(Uri uri,String format)throws Exception{
        long end=session.active()?System.currentTimeMillis():session.effectiveEnd();List<TelemetryHistoryStore.Sample> samples=telemetry.query(session.startTs,end,0);
        OutputStream os=getContentResolver().openOutputStream(uri);if(os==null)throw new IllegalStateException("Не удалось открыть файл");
        try(OutputStreamWriter w=new OutputStreamWriter(os,"UTF-8")){if("csv".equals(format))writeCsv(w,samples);else writeJson(w,samples);}
    }

    private void writeCsv(OutputStreamWriter w,List<TelemetryHistoryStore.Sample> samples)throws Exception{
    ...
    private void writeJson(OutputStreamWriter w,List<TelemetryHistoryStore.Sample> samples)throws Exception{
        JSONObject root=new JSONObject(),summary=new JSONObject();
        summary.put("source",isTestSession(session)?"test":"live");summary.put("start_ts",session.startTs);summary.put("end_ts",session.effectiveEnd());summary.put("duration_ms",session.durationMs());summary.put("samples",session.samples);summary.put("outage_ms",session.outageMs);
        put(summary,"camera_min",session.cameraMin);put(summary,"camera_max",session.cameraMax);put(summary,"probe_k_min",session.kMin);put(summary,"probe_k_max",session.kMax);put(summary,"probe_t_min",session.tMin);put(summary,"probe_t_max",session.tMax);put(summary,"heater_max",session.heaterMax);root.put("session",summary);
        JSONArray a=new JSONArray();for(TelemetryHistoryStore.Sample x:samples){JSONObject o=new JSONObject();o.put("ts",x.ts);put(o,"camera",x.camera);put(o,"setpoint",x.setpoint);put(o,"probe_k",x.probeK);put(o,"probe_t",x.probeT);put(o,"heater",x.heater);a.put(o);}root.put("telemetry",a);w.write(root.toString(2));
    }
