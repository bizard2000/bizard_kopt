...
/** Long-running owner of Bluetooth, Android Auto, MQTT and telemetry-confirmed command ACKs. */
public class HomeSmokeService extends Service {
    ...
    private final ExecutorService mqttIo=Executors.newSingleThreadExecutor(r -> new Thread(r,"HomeSmoke-MQTT-io"));
    ...
    public void connectBluetooth(String address){
        startService(new Intent(this,HomeSmokeService.class));closeBluetooth(false);lastError="";emit();
        btThread=new Thread(()->{
            try{
    ...
    private void publishAck(String id,String cmd,boolean ok,String state,double value){mqttIo.execute(()->publishAckNow(id,cmd,ok,state,value));}
    private void publishAckNow(String id,String cmd,boolean ok,String state,double value){MqttClient m=mqtt;if(m==null||!m.isConnected())return;try{JSONObject o=new JSONObject();o.put("v",2);o.put("id",id);o.put("device_id",deviceId());o.put("ts",System.currentTimeMillis());o.put("cmd",cmd);o.put("ok",ok);o.put("state",state);o.put("message",state);if(!Double.isNaN(value))o.put("value",value);m.publish(ackTopic(),o.toString(),false,0);}catch(Exception e){mqttTransportFailure(m,"MQTT ACK: "+safe(e));}}
    ...
    private void updateForeground(){
        ...
        if(need){if(!foreground)enterForeground();else refreshNotification();}
